...
def detect(model, img, imgsz, image_base_name):
    current = image_base_name
        # Get img shape
   # height, width, channels = img.shape
    results = model.predict(source=img.copy(), project="Flask_Results", name=f"pred_{current}", overlap_mask=False, imgsz=imgsz, save=True, exist_ok=True, iou=0.8, conf=0.6, save_txt=False)
    result = results[0]
...
def application():
    if request.method == 'POST':
        ...
        xls_filepath = os.path.join(f'Flask_Results/pred_{image_base_name}', xls_filename)
        df1.to_excel(xls_filepath)
        
        bbox_img = cv2.imread(f'Flask_Results/pred_{image_base_name}/image0.jpg')
        bbox_img_filename = image_base_name + "_bboxPred.jpg" 
        bbox_img_path = os.path.join(f'Flask_Results/pred_{image_base_name}', bbox_img_filename)
        cv2.imwrite(bbox_img_path, bbox_img)
        bbox_img_path1 = os.path.join('static/Result/prediction', bbox_img_filename)
        cv2.imwrite(bbox_img_path1, bbox_img)
